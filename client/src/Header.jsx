...
const Header = ()=>{
  const {user} = useContext(UserContext)
    return(
      ...
      <div className="flex border border-gray-300 rounded-full gap-2 p-2 shadow-md shadow-gray-300"> 
        <div>Anywhere</div>
      ...
      <Link to={user? '/account':'/login'} className="flex border border-gray-300 rounded-full gap-2 p-2">
        ...
        {
          user && <div className="text-gray-500">{user.username}</div>
        }                        
      </Link>
    </header>
    )
